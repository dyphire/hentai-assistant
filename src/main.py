...
from utils import get_task_logger, check_dirs
import nfotool
...
def check_config():
    ...
    if 'enable' in config_parser['aria2'] and config_parser['aria2']['enable'].lower() in ['true', '1', 'yes']: 
        ...
        rpc = aria2.Aria2RPC(url=app.config['aria2_server'] ,token=app.config['aria2_token'], logger=logger)
        try:
            ...
            logger.error(f"Aria2 RPC 连接异常: {e}")
            aria2_toggle = False
    else:  
    ...
    if 'enable' in config_parser['komga'] and config_parser['komga']['enable'].lower() in ['true', '1', 'yes']:
        ...
        if library.status_code == 200:
            ...
            logger.info(f"{library.json()['name']} {library.json()['id']} {library.json()['root']}")
        else:
            komga_toggle = False
            logger.error("Komga API 连接异常, 相关功能将不可用")
    else:
...
def download_url():
    ...
    future = executor.submit(download_task, url, task_id, logger=get_task_logger(task_id)) 
    return jsonify({'message': f"Download task for {url} started with task ID {task_id}."}), 202
